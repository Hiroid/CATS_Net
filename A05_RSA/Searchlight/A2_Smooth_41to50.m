...
Subject = dir([RootFolder filesep 'A1_fishZ_all_eval_TransE_41to50' filesep 'SUB*']);
nSub = length(Subject);
...
for s=1:nSub
    ...
    mkdir([RootFolder filesep 'matlabbatch_eval_TransE_41to50']);
    save([RootFolder filesep 'matlabbatch_eval_TransE_41to50' filesep 's_' Subject(s).name '.mat'],'matlabbatch');
...
RootFolder='A1_Searchlight';
Source=[RootFolder filesep 'A1_fishZ_all_eval_TransE_41to50'];
...
for i=1:length(Imgs)
    mkdir([RootFolder filesep 'A2_Smooth_fishZ_all_eval_TransE_41to50' filesep Imgs(i).name(1:end-4)]);
    for subj=1:nSub
        if exist([Source filesep Subject(subj).name filesep Imgs(i).name],'file')
            copyfile([Source filesep Subject(subj).name filesep Imgs(i).name], [RootFolder filesep 'A2_Smooth_fishZ_all_eval_TransE_41to50' filesep Imgs(i).name(1:end-4) filesep Imgs(i).name(1:end-4) '_' Subject(subj).name '.nii']);
        end
    end
end
